...
- Input [2055492159](https://ankiweb.net/shared/info/2055492159) into the text box labeled `Code` and press the `OK` button to proceed.
...
If you have done so, you should see the [`requestPermission`](https://foosoft.net/projects/anki-connect/index.html#requestpermission) API request return `true` for `requireApiKey`.
...
Search parameters are passed to Anki, check the docs for more information: [https://docs.ankiweb.net/searching.html](https://docs.ankiweb.net/searching.html)
- [Card Actions](https://foosoft.net/projects/anki-connect/index.html#card-actions)
- [Deck Actions](https://foosoft.net/projects/anki-connect/index.html#deck-actions)
- [Graphical Actions](https://foosoft.net/projects/anki-connect/index.html#graphical-actions)
- [Media Actions](https://foosoft.net/projects/anki-connect/index.html#media-actions)
- [Miscellaneous Actions](https://foosoft.net/projects/anki-connect/index.html#miscellaneous-actions)
- [Model Actions](https://foosoft.net/projects/anki-connect/index.html#model-actions)
- [Note Actions](https://foosoft.net/projects/anki-connect/index.html#note-actions)
- [Statistic Actions](https://foosoft.net/projects/anki-connect/index.html#statistic-actions)
...
```json
{ "action": "guiSelectNote", "version": 6, "params": { "note": 1494723142483 } }
```
...
Invokes the *Add Cards* dialog, presets the note using the given deck and model, with the provided field values and tags. Invoking it multiple times closes the old window and *reopen the window* with the new provided values. Audio, video, and picture files can be embedded into the fields via the `audio`, `video`, and `picture` keys, respectively. Refer to the documentation of `addNote` and `storeMediaFile` for an explanation of these fields. The result is the ID of the note which would be added, if the user chose to confirm the *Add Cards* dialogue.
...
Repositions a template in an existing model. The value of `index` starts at 0. For example, an index of `0` puts the template in the first position, and an index of `2` puts the template in the third position.
...
Reposition the field within the field list of a given model. The value of `index` starts at 0. For example, an index of `0` puts the field in the first position, and an index of `2` puts the field in the third position.
...
Creates a new field within a given model. Optionally, the `index` value can be provided, which works exactly the same as the index in `modelFieldReposition`. By default, the field is added to the end of the field list.
...
Sets the description (the text seen in the gui editor when a field is empty) for a field within a given model. Older versions of Anki (2.1.49 and below) do not have field descriptions. In that case, this will return with `false`.
...
```json
{ "action": "notesModTime", "version": 6, "params": { "notes": [1502298033753] } }
```

**Sample result:**

```json
{ "result": [ { "noteId": 1498938915662, "mod": 1629454092 } ], "error": null }
```
...
```json
{ "action": "removeEmptyNotes", "version": 6 }
```
...
```json
{ "result": [ ["2021-02-28", 124], ["2021-02-27", 261] ], "error": null }
```
...
```json
{ "action": "getCollectionStatsHTML", "version": 6, "params": { "wholeCollection": true } }
```
...
```json
{ "action": "getReviewsOfCards", "version": 6, "params": { "cards": [ "1653613948202" ] } }
```

**Sample result:**

```json
{ "result": { "1653613948202": [ { "id": 1653772912146, "usn": 1750, "ease": 1, "ivl": -20, "lastIvl": -20, "factor": 0, "time": 38192, "type": 0 }, { "id": 1653772965429, "usn": 1750, "ease": 3, "ivl": -45, "lastIvl": -20, "factor": 0, "time": 15337, "type": 0 } ] }, "error": null }
```
...
```json
{ "action": "getLatestReviewID", "version": 6, "params": { "deck": "default" } }
```

**Sample result:**

```json
{ "result": 1594194095746, "error": null }
```
...
```json
{ "action": "insertReviews", "version": 6, "params": { "reviews": [ [1594194095746, 1485369733217, -1, 3, 4, -60, 2500, 6157, 0], [1594201393292, 1485369902086, -1, 1, -60, -60, 0, 4846, 0] ] } }
```
